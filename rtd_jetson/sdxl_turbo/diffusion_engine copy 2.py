...
from diffusers.utils import USE_PEFT_BACKEND, BaseOutput, deprecate, logging, scale_lora_layers, unscale_lora_layers, load_image
import matplotlib
import gc
...
class DiffusionEngine():
    ...
    def __init__(
        ...
        self.use_controllnet = use_controllnet

        if self.use_image2image:
            self._init_image2image()
        elif self.use_controllnet:
            self._init_controllnetdepth()
        else:
    ...
    def _init_controllnetdepth(self):
        print("Initializing ControlNet Depth SDXL model...")
        try:
            print("Loading ControlNet Depth SDXL model...")
            controlnet = ControlNetModel.from_pretrained(
                "diffusers/controlnet-depth-sdxl-1.0-small", torch_dtype=torch.float16
            )
            print("ControlNet Depth SDXL model loaded successfully.")
            print("Loading VAE model...")
            vae=AutoencoderKL.from_pretrained("madebyollin/sdxl-vae-fp16-fix", torch_dtype=torch.float16)
            print("VAE model loaded successfully.") 
            print("Loading StableDiffusionXLControlNetPipeline...")
            ...
            pipe.to(self.device)
            print("StableDiffusionXLControlNetPipeline loaded successfully.")
        except Exception as e:
            print("Loading ControlNet Depth SDXL model...")
            controlnet = ControlNetModel.from_pretrained(
    ...
    def set_controllnet_scale(self, controllnet_scale):
        """
        This method sets the ControlNet scale for the pipeline. The scale is used to adjust the influence of the ControlNet conditioning.
        
        Args:
            controllnet_scale (float): The scale value for ControlNet conditioning.
        """
        self.controllnet_scale = float(controllnet_scale)
        if self.use_controllnet:
            self.pipe.controlnet_conditioning_scale = self.controllnet_scale
    ...
    def build_kwargs(self, kwargs_override=None):
        ...
        if self.use_controllnet:
            kwargs.update({
                'controlnet_conditioning_scale': self.controllnet_scale,
                'image': self.image_init,
            })
    ...
    def build_cross_attention_kwargs(self, kwargs, cross_attention_kwargs_override=None):
        ...
        # Note: controlnet_conditioning_scale is now passed as a top-level parameter, not through cross_attention_kwargs
        ...
        if len(cross_attention_kwargs) > 0:
            print(f"[DiffusionEngine] cross_attention_kwargs: {cross_attention_kwargs}")
            print(f"[DiffusionEngine] cross_attention_kwargs keys: {list(cross_attention_kwargs.keys())}")
    ...
    def generate(self, kwargs_override=None, cross_attention_kwargs_override=None):
        ...
        print(f"[DiffusionEngine] generate called. ControlNet: {getattr(self, 'controlnet', None)}")
        print(f"[DiffusionEngine] Built kwargs keys: {list(kwargs.keys())}")
        if 'controlnet' in kwargs:
            print(f"[DiffusionEngine] ControlNet in kwargs: {kwargs['controlnet']}")
        if 'control_image' in kwargs:
            print(f"[DiffusionEngine] Control image in kwargs: {type(kwargs['control_image'])}")
        if 'controlnet_conditioning_scale' in kwargs:
            print(f"[DiffusionEngine] ControlNet scale in kwargs: {kwargs['controlnet_conditioning_scale']}")
...
if __name__ == '__main__':
    matplotlib.use('TkAgg')  # Use TkAgg to allow window display
    import matplotlib.pyplot as plt

    print("DiffusionEngine example usage:")
    # Example for overriding kwargs
    height_diffusion = 704
    width_diffusion = 704
    depth_image_path1 = "/home/lugo/ComfyUI/custom_nodes/PIC-BitalinoComfy/src/im_process/midas_test_results/sphere_render_MiDaS_small_depth.png"
    depth_image_path2 = "/home/lugo/ComfyUI/custom_nodes/PIC-BitalinoComfy/src/im_process/midas_test_results/image_MiDaS_small_depth.png"
    depth_image_path3 = "/home/lugo/.config/Code/User/workspaceStorage/vscode-chat-images/image-1748630122753.png"
    depth_map1 = load_image(depth_image_path1)
    depth_map2 = load_image(depth_image_path2)
    depth_map3 = load_image(depth_image_path3)

    depth_maps = [depth_map1, depth_map2, depth_map3]
    depth_map_names = ["depth_map1", "depth_map2", "depth_map3"]

    prompts = [
        "beautiful mystic 4k blackhole",
        "a futuristic cityscape at sunset",
        "a serene mountain landscape with a lake"
    ]

    # Use three scales: 0, 0.5, 1
    scales = [0.0, 0.5, 1.0]

    print(f"Diffusion engine initializing with height {height_diffusion} and width {width_diffusion}")
    de_txt = DiffusionEngine(use_controllnet=True, height_diffusion_desired=height_diffusion, width_diffusion_desired=width_diffusion)
    em = EmbeddingsMixer(de_txt.pipe)

    # Each row: one depth map, columns: prompt1, prompt2, prompt3, plus 1 for showing the depth map, times number of scales
    n_rows = len(depth_maps)
    n_cols = len(prompts) * len(scales) + 1  # 1 for showing the depth map

    fig, axs = plt.subplots(n_rows, n_cols, figsize=(5 * n_cols, 5 * n_rows))

    # Store all generated images for plotting
    all_images = [[None for _ in range(n_cols)] for _ in range(n_rows)]

    for row_idx, (depth_map, name) in enumerate(zip(depth_maps, depth_map_names)):
        # First column: show the depth map
        all_images[row_idx][0] = np.array(depth_map)
        # For each prompt and each scale, generate images
        for prompt_idx, prompt in enumerate(prompts):
            for scale_idx, scale in enumerate(scales):
                de_txt.set_embeddings(em.encode_prompt(prompt))
                de_txt.set_input_image(depth_map)
                de_txt.set_guidance_scale(scale)
                kwargs_override = {"num_inference_steps": 2, "controlnet_conditioning_scale": scale}
                img = de_txt.generate(kwargs_override=kwargs_override)
                if not isinstance(img, np.ndarray):
                    img = np.array(img)
                # Calculate column: 1 + prompt_idx * len(scales) + scale_idx
                col = 1 + prompt_idx * len(scales) + scale_idx
                all_images[row_idx][col] = img
    ...
    for row_idx in range(n_rows):
        for col in range(n_cols):
            if n_rows == 1:
                ...
                ax = axs[row_idx, col]
            ax.imshow(all_images[row_idx][col])
            if col == 0:
                ax.set_title(f"Depth {depth_map_names[row_idx]}")
            else:
                # Figure out which prompt and scale this is
                prompt_idx = (col - 1) // len(scales)
                scale_idx = (col - 1) % len(scales)
                ax.set_title(f"Prompt: {prompts[prompt_idx]}\nScale: {scales[scale_idx]}")
            ax.axis('off')
