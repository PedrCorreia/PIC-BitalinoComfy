#!/usr/bin/env python
"""
PlotUnit Standalone Debug App with Proper Constants Import

This application imports constants from the project's module structure
while keeping the implementation self-contained for easy debugging of
button clicks and tab navigation.
"""
...
# Initialize default constants (will be overridden if import succeeds)
...
try:
    ...
    print("\nSuccessfully imported constants from plot.constants")
    IMPORTED_CONSTANTS = True
except ImportError:
    try:
        # Approach 2: Fully qualified import
        from src.plot.constants import *
        print("\nSuccessfully imported constants from src.plot.constants")
        IMPORTED_CONSTANTS = True
    except ImportError:
        try:
            # Approach 3: Manually load the constants module
            import importlib.util
            constants_path = os.path.join(SRC_DIR, 'plot', 'constants.py')
            if os.path.exists(constants_path):
                spec = importlib.util.spec_from_file_location("constants", constants_path)
                constants = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(constants)
                # Copy all attributes from the constants module to the global namespace
                for attr_name in dir(constants):
                    if not attr_name.startswith('__'):
                        globals()[attr_name] = getattr(constants, attr_name)
                print(f"\nSuccessfully imported constants from file: {constants_path}")
                IMPORTED_CONSTANTS = True
            else:
                print(f"\nConstants file not found at {constants_path}")
                IMPORTED_CONSTANTS = False
        except Exception as e:
            print(f"\nFailed to import constants: {e}")
            IMPORTED_CONSTANTS = False

# Print some key constants to verify they were properly imported
if IMPORTED_CONSTANTS:
    print(f"Imported WINDOW_WIDTH = {WINDOW_WIDTH}")
    print(f"Imported STATUS_BAR_TOP = {STATUS_BAR_TOP}")
    print(f"Imported BUTTON_COLOR_SETTINGS = {BUTTON_COLOR_SETTINGS}")
else:
    print("Using fallback constants")

# Define placeholder for the ViewMode enum
class ViewMode(Enum):
    RAW = 0
    PROCESSED = 1
    TWIN = 2
...
# Try to import the ViewMode enum
try:
    from plot.view_mode import ViewMode
    print("\nSuccessfully imported ViewMode from plot.view_mode")
    IMPORTED_VIEW_MODE = True
except ImportError:
    try:
        from src.plot.view_mode import ViewMode
        print("\nSuccessfully imported ViewMode from src.plot.view_mode")
        IMPORTED_VIEW_MODE = True
    except ImportError:
        print("\nFailed to import ViewMode, using local definition")
        IMPORTED_VIEW_MODE = False

# Now try to import UI components
IMPORTED_COMPONENTS = False
IMPORTED_COMPONENT_PATHS = {}

try:
    print("\nAttempting to import UI components...")
    ...
    # First approach - direct imports
    try:
        # Try each component individually to provide detailed feedback
        component_paths = {
            "BaseView": "plot.view.base_view",
            "SettingsView": "plot.view.settings_view",
            "Sidebar": "plot.ui.sidebar",
            "StatusBar": "plot.ui.status_bar",
            "EventHandler": "plot.event_handler"
        }
        ...
        for component, path in component_paths.items():
            try:
                ...
                IMPORTED_COMPONENT_PATHS[component] = path
                imported_count += 1
                print(f"  ✓ Imported {component} from {path}")
            except (ImportError, AttributeError) as e:
                print(f"  ✗ Failed to import {component} from {path}: {e}")
        
        if imported_count == len(component_paths):
            print("Successfully imported all UI components from plot module")
            IMPORTED_COMPONENTS = True
        else:
            print(f"Imported {imported_count}/{len(component_paths)} components from direct paths")
            
    except Exception as e:
        print(f"Error during direct imports: {e}")
        
        # Second approach - qualified imports
        try:
            # Try each component individually with qualified imports
            component_paths = {
                "BaseView": "src.plot.view.base_view",
                "SettingsView": "src.plot.view.settings_view",
                "Sidebar": "src.plot.ui.sidebar",
                "StatusBar": "src.plot.ui.status_bar",
                "EventHandler": "src.plot.event_handler"
            }
            ...
            for component, path in component_paths.items():
                try:
                    if component not in globals():  # Only import if not already imported
                        module = importlib.import_module(path)
                        globals()[component] = getattr(module, component)
                        IMPORTED_COMPONENT_PATHS[component] = path
                        imported_count += 1
                        ...
                        print(f"  ℹ {component} was already imported")
                except (ImportError, AttributeError) as e:
                    print(f"  ✗ Failed to import {component} from {path}: {e}")
            
            if len(IMPORTED_COMPONENT_PATHS) == len(component_paths):
                print("Successfully imported all UI components")
                IMPORTED_COMPONENTS = True
            else:
                print(f"Imported {len(IMPORTED_COMPONENT_PATHS)}/{len(component_paths)} components total")
                if len(IMPORTED_COMPONENT_PATHS) > 0:
                    IMPORTED_COMPONENTS = True  # Consider partial success as success
        except Exception as e:
            print(f"Error during qualified imports: {e}")
except Exception as e:
    print(f"Error during component import process: {e}")
    
# Summarize import results
print("\nComponent Import Summary:")
if IMPORTED_COMPONENTS:
    print(f"Successfully imported {len(IMPORTED_COMPONENT_PATHS)} components:")
    for component, path in IMPORTED_COMPONENT_PATHS.items():
        print(f"  - {component} from {path}")
else:
    print("Failed to import UI components, will use local implementations")
...
class PlotUnit:
    ...
    def get_instance(cls):
      ...
      def __init__(self):
        ...
        # Print component import status
        print(f"Component imports enabled: {IMPORTED_COMPONENTS}")
        print(f"ViewMode import enabled: {IMPORTED_VIEW_MODE}")
    ...
    def start(self):
      ...
      def _run(self):
        ...
        try:            # Initialize pygame
            ...
            if IMPORTED_COMPONENTS:
                print("Using imported UI component classes")
                self.sidebar = Sidebar(self.surface, self.font)
                self.status_bar = StatusBar(self.surface, self.font)
                      # Create views using imported component classes                try:
                    # Try to use specific view implementations if available
                    ...
                    print("Using specialized view implementations")
                    self.views = {
                    ...
                    # Fall back to BaseView for all views if specialized ones aren't available
                    print("Specialized views not found, using BaseView for all views")
                    self.views = {
                ...
                print("Successfully initialized imported UI components")
            else:
                print("Using local UI component classes")
                self.sidebar = Sidebar(self.surface, self.font)
                self.status_bar = StatusBar(self.surface, self.font)
                      # Create views with local implementations
                self.views = {
                    ViewMode.RAW: BaseView(self.surface, self.data_lock, self.data, self.font),
                    ViewMode.PROCESSED: BaseView(self.surface, self.data_lock, self.data, self.font),
                    ViewMode.TWIN: BaseView(self.surface, self.data_lock, self.data, self.font),
            ...
            if IMPORTED_COMPONENTS and 'EventHandler' in globals():
                print("Using imported EventHandler")
                self.event_handler = EventHandler(self.sidebar, self.views[ViewMode.SETTINGS])
            else:
                print("Using local EventHandler implementation")
                self.event_handler = EventHandler(self.sidebar, self.views[ViewMode.SETTINGS])
