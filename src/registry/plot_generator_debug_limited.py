#!/usr/bin/env python
"""
Plot Generator Debug - Registry Integration (With Signal Limits)
...
This version enforces the system constraint of max 3 raw signals and 3 processed signals.
"""
...
class RegistrySignalGenerator:
    ...
    """    def __init__(self):
        ...
        self.generators = {
            'raw': {
                ...
                'SINE': {'created': False, 'id': None}
            },
            'processed': {
                'CustomWave1': {'created': False, 'id': None},
                'CustomWave2': {'created': False, 'id': None},
                'CustomWave3': {'created': False, 'id': None}
            }
        }
    ...
    def _generate_signals(self):
        ...
        # Create initial signals (respect limits)
        self._create_ecg_signal()  # Raw signal 1
        self._create_eda_signal()  # Raw signal 2
        self._create_synthetic_signal("SINE", self._generate_sine, processed=True)  # Processed signal 1
    ...
    def _create_ecg_signal(self):
        ...
        # Check if we've hit the raw signal limit
        if len(self.raw_signals) >= self.raw_signal_limit:
            print(f"WARNING: Cannot create ECG signal - reached raw signal limit of {self.raw_signal_limit}")
            return None
        ...
        self.raw_signals.append(signal_id)
        
        print(f"Created ECG signal with ID: {signal_id} (Raw signal {len(self.raw_signals)}/{self.raw_signal_limit})")
        return signal_id
    ...
    def _create_eda_signal(self):
        ...
        # Check if we've hit the raw signal limit
        if len(self.raw_signals) >= self.raw_signal_limit:
            print(f"WARNING: Cannot create EDA signal - reached raw signal limit of {self.raw_signal_limit}")
            return None
        ...
        print(f"Created EDA signal with ID: {signal_id} (Raw signal {len(self.raw_signals)}/{self.raw_signal_limit})")
        return signal_id
        
    def _create_synthetic_signal(self, name, generator_func, processed=False):
        """
        Create a custom synthetic signal in the registry.
        
        Args:
            name: Name of the signal
            generator_func: Function that generates signal values
            processed: If True, this will be counted as a processed signal, otherwise raw
        """
        # Check signal limits based on type
        if processed:
            if len(self.processed_signals) >= self.processed_signal_limit:
                print(f"WARNING: Cannot create {name} signal - reached processed signal limit of {self.processed_signal_limit}")
                return None
        else:
            if len(self.raw_signals) >= self.raw_signal_limit:
                print(f"WARNING: Cannot create {name} signal - reached raw signal limit of {self.raw_signal_limit}")
                return None
        
        # Create ID with appropriate prefix for signal type
        prefix = "PROC" if processed else "SYN"
        signal_id = f"{prefix}_{name}"
        initial_data = np.array([])
        
        # Define metadata
        metadata = {
            'name': f'{"Processed" if processed else "Synthetic"} {name}',
        ...
        # Store in active signals and type-specific list
        self.signals[signal_id] = metadata
        if processed:
            self.processed_signals.append(signal_id)
            print(f"Created {name} signal with ID: {signal_id} (Processed signal {len(self.processed_signals)}/{self.processed_signal_limit})")
        else:
            self.raw_signals.append(signal_id)
            print(f"Created {name} signal with ID: {signal_id} (Raw signal {len(self.raw_signals)}/{self.raw_signal_limit})")
    ...
    def add_custom_signal(self, name, processed=False):
        ...
        Returns:
            str: Signal ID if successful, None if limit reached
        """
        # Sine wave generator with slight variation based on the name
        def custom_generator(elapsed_time, sample_index):
            # Use hash of name to vary the frequency slightly (0.3-0.7 Hz range)
            name_hash = sum(ord(c) for c in name) % 100
            freq_variation = 0.3 + (name_hash / 100.0) * 0.4  # 0.3-0.7 Hz
            
            # Generate sine wave with the custom frequency
            t = elapsed_time
            value = np.sin(2 * np.pi * freq_variation * t)
            
            # Add some noise
            value = value * 0.9 + np.random.normal(0, 0.1)
            return value
            
        # Create a signal with the custom generator
        return self._create_synthetic_signal(name, custom_generator, processed=processed)
...
def run_demo(duration=600):
    ...
    """
    print("\n=== Starting PIC-2025 Registry Visualization Demo (Signal Limited) ===\n")
    ...
    generator.start()
    print("Signal generator started, registering signals with limits (3 raw, 3 processed)...")
    ...
    try:
        ...
        # Demonstrate adding a custom processed signal after 5 seconds
        start_time = time.time()
        custom_signal_added = False
        custom_signal2_added = False
        ...
        while time.time() - start_time < duration:
            ...
            if elapsed >= 5 and not custom_signal_added:
                print("\nAdding a processed signal (1/3)...")
                custom_id = generator.add_custom_signal("CustomWave1", processed=True)
                if custom_id:
                    print(f"Added custom processed signal: {custom_id}")
                else:
                    print("Failed to add custom processed signal (limit reached)")
                custom_signal_added = True
                
            # Add another processed signal after 10 seconds
            if elapsed >= 10 and not custom_signal2_added:
                print("\nAdding another processed signal (2/3)...")
                custom_id = generator.add_custom_signal("CustomWave2", processed=True)
                if custom_id:
                    print(f"Added custom processed signal: {custom_id}")
                else:
                    print("Failed to add custom processed signal (limit reached)")
                custom_signal2_added = True
            
            # Show signal counts every 5 seconds
            if elapsed % 5 == 0 and elapsed > 0:
                raw_count = len(generator.raw_signals)
                proc_count = len(generator.processed_signals)
                print(f"Running demo... {elapsed}/{duration} seconds - {raw_count}/3 raw signals, {proc_count}/3 processed signals active")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            run_demo(duration=duration)
        except ValueError:
            print(f"Invalid duration argument: {sys.argv[1]}. Using default.")
            run_demo(duration=600)  # Run for 10 minutes by default
    else:
        run_demo(duration=600)  # Run for 10 minutes by default
