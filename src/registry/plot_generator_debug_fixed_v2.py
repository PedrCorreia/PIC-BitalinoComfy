#!/usr/bin/env python
"""
Plot Generator Debug - Registry Integration (Fixed Version v2)
...
This version fixes:
1. Connection management when switching views
2. Signal data buffering implementation
3. GUI freezing issues
4. View mode transition crashes
"""
...
class RegistrySignalGenerator:
    ...
    def __init__(self):
        ...
        # Last generated values for each signal (to handle view switching without data loss)
    ...
    def get_active_signal_ids_by_type(self, signal_type):
        """
        Get active signal IDs by type (raw or processed)
        
        Args:
            signal_type: Type of signals to get ('raw' or 'processed')
            
        Returns:
            Set of signal IDs matching the type
        """
        return {
...
class PlotUnitRegistryAdapter:
    ...
    def __init__(self, plot_unit=None):
        ...
        try:
            ...
            self.latency_values = []
            self.latency_window_size = 20  # Keep track of last 20 latency values
            
            # Connection management
            self.connected_signals = set()  # Track which signals are actively connected
    ...
    def disconnect(self):
        ...
        Args:
            view_mode: The ViewMode enum value
        """
        if not VIEW_MODE_IMPORTED:
            print("ViewMode enum not imported, cannot filter by view mode")
            return
        
        # Store previous view mode for transition handling
        previous_view_mode = self.current_view_mode
        self.current_view_mode = view_mode
        
        # Update active signal types based on view mode
        if view_mode == ViewMode.RAW:
            self.active_signal_types = {'raw'}
            print("Set adapter to RAW view mode - showing only raw signals")
            
            # Special handling for RAW view to fix glitching
            if hasattr(self.plot_unit, 'data_lock') and hasattr(self.plot_unit, 'data'):
                with self.plot_unit.data_lock:
                    # First ensure we have a clean state by removing all processed signals
                    for signal_id in list(self.plot_unit.data.keys()):
                        if signal_id.startswith("PROC_"):
                            del self.plot_unit.data[signal_id]
                            print(f"Cleaned up processed signal {signal_id} from RAW view")
                
                # For RAW view, don't immediately force a render as it can cause glitching
                # Let the normal render cycle handle it
                print("Applied anti-glitch protection for RAW view")
                        
        elif view_mode == ViewMode.PROCESSED:
            self.active_signal_types = {'processed'}
            print("Set adapter to PROCESSED view mode - showing only processed signals")
            
            # Clear any raw signals from PlotUnit data when switching to PROCESSED
            if previous_view_mode != view_mode and hasattr(self.plot_unit, 'data_lock') and hasattr(self.plot_unit, 'data'):
                with self.plot_unit.data_lock:
                    # Find all raw signals in current data
                    raw_signals = [signal_id for signal_id in list(self.plot_unit.data.keys()) 
                                  if signal_id in self.connected_signals and 
                                  not signal_id.startswith("PROC_")]
                    
                    # Remove raw signals from data
                    for signal_id in raw_signals:
                        if signal_id in self.plot_unit.data:
                            del self.plot_unit.data[signal_id]
                            print(f"Removed raw signal {signal_id} from view")
                    
                    # Ensure the UI refreshes by forcing a redraw
                    if hasattr(self.plot_unit, '_render'):
                        self.plot_unit._render()
                        
        else:
            # For other view modes, show all signals
            self.active_signal_types = {'raw', 'processed'}
            print(f"Set adapter to {view_mode.name} view mode - showing all signals")
        
    def _should_include_signal(self, signal_id, metadata):
        """
        Determine if a signal should be included based on current view mode.
        
        Args:
            signal_id: The signal ID
            metadata: The signal metadata
            
        Returns:
            bool: True if the signal should be included, False otherwise
        """
        # If no view mode filtering is active, include all signals
        if not self.current_view_mode or not VIEW_MODE_IMPORTED:
            return True
            
        # Get signal type from metadata
        signal_type = metadata.get('type', 'unknown')
        
        # Check if this signal type is active
        return signal_type in self.active_signal_types
    def _get_signals_for_current_view(self):
        """
        Get all signals that should be displayed in the current view mode.
        
        Returns:
            dict: Dictionary of signal_id -> {data, metadata} for current view mode
        """
        signals_to_render = {}
        lock_to_use = None  # Initialized to avoid scope issues
        ...
        try:
            ...
            for signal_id in signal_ids:
                ...
                if signal_data is not None:
                    ...
                    # Check if this signal should be included in current view
                    if self._should_include_signal(signal_id, metadata):
    ...
    def _monitor_registry(self):
        ...
        # Track if this is the first update since switching view modes
        last_view_mode = None
        view_mode_changed = False
        last_raw_update_time = 0
        ...
        while self.running:
            try:
                # Track current time for latency measurement
                monitor_start_time = time.time()
                
                # Check if view mode has changed
                if self.current_view_mode != last_view_mode:
                    view_mode_changed = True
                    last_view_mode = self.current_view_mode
                    print(f"View mode changed to: {self.current_view_mode.name if self.current_view_mode else 'None'}")
                
                # Get signals for current view
                signals_to_render = self._get_signals_for_current_view()
                ...
                # Get the current list of signal IDs to check for connections/disconnections
                current_signal_ids = set(signals_to_render.keys())
                
                # Check for newly connected signals
                new_signals = current_signal_ids - self.connected_signals
                for signal_id in new_signals:
                    print(f"New signal connected: {signal_id}")
                    self.connected_signals.add(signal_id)
                
                # Check for disconnected signals
                disconnected_signals = self.connected_signals - current_signal_ids
                for signal_id in disconnected_signals:
                    print(f"Signal disconnected: {signal_id}")
                    # Remove from PlotUnit data to prevent rendering stale data
                    if hasattr(self.plot_unit, 'data_lock') and hasattr(self.plot_unit, 'data'):
                        with self.plot_unit.data_lock:
                            if signal_id in self.plot_unit.data:
                                del self.plot_unit.data[signal_id]
                    self.connected_signals.remove(signal_id)
                ...
                # Calculate latency for this update cycle
                update_latency = time.time() - monitor_start_time
                self.latency_values.append(update_latency)
                
                # Keep latency window at desired size
                if len(self.latency_values) > self.latency_window_size:
                ...
                if hasattr(self.plot_unit, 'status_bar') and hasattr(self.plot_unit.status_bar, 'draw'):
                    try:
                        ...
                        try:
                            ...
                            print(f"Warning: Status bar update failed: {e2}")
                
                # Wait a short time before checking again - adaptive based on latency
                # If latency is high, check less frequently to avoid overloading
                wait_time = max(0.05, min(0.2, avg_latency * 2))  # Between 50ms and 200ms
                time.sleep(wait_time)                
            except Exception as e:
                print(f"Error in registry monitoring thread: {e}")
                traceback.print_exc()
                
                # Release lock if we're holding it during an exception
                if 'lock_to_use' in locals() and lock_to_use and lock_to_use.locked():
                    try:
                        lock_to_use.release()
                    except RuntimeError:
...
def run_demo(duration=600, buffer_seconds=10):
    ...
    """
    print("\n=== Starting PIC-2025 Registry Visualization Demo (Fixed Version v2) ===\n")
    ...
    # If ViewMode is available, connect view mode change handler
    if VIEW_MODE_IMPORTED and hasattr(plot_unit, 'current_mode'):
        # Set adapter's view mode based on PlotUnit's current mode
        adapter.set_view_mode(plot_unit.current_mode)
        
        # Patch the _set_mode method to update adapter when mode changes
        original_set_mode = plot_unit._set_mode
        
        def patched_set_mode(mode):
            # Call original method
            original_set_mode(mode)
            # Update adapter's view mode
            adapter.set_view_mode(mode)
        
        # Replace the method
        plot_unit._set_mode = patched_set_mode
        print("Connected view mode change handler to adapter")
    ...
    try:
        ...
        while time.time() - start_time < duration:
            ...
            if elapsed % 5 == 0 and elapsed > 0:
                ...
                latency_info = ""
                if adapter.latency_values:
                    avg_latency = sum(adapter.latency_values) / len(adapter.latency_values)
                    latency_info = f", avg latency: {avg_latency:.3f}s"
                
                print(f"Running demo... {elapsed}/{duration} seconds - " 
                      f"{raw_count}/3 raw signals, {proc_count}/3 processed signals, "
                      f"{generator.buffer_seconds}s buffer{latency_info}")
            
            # Suggest trying different view modes every 15 seconds
            if elapsed % 15 == 0 and elapsed > 0:
                print("\nTip: Try switching between RAW and PROCESSED views using the sidebar buttons!")
