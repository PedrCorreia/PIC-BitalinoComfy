#!/usr/bin/env python3
"""
Test script for displaying GeometryRenderNode output in the lunar_tools display window.
This continuously renders different 3D geometries in a background thread
and displays them in the lunar_tools window while keeping the window responsive.
"""
...
# Import lunar_tools display window directly from the specified path
lunar_tools_comfy_path = os.path.join(os.path.dirname(script_dir), "lunar_tools_comfy")
lunar_display_path = os.path.join(lunar_tools_comfy_path, "comfy")
HAS_LUNAR_TOOLS = False

if os.path.exists(lunar_display_path):
    sys.path.append(lunar_tools_comfy_path)
    try:
        from lunar_tools_comfy.comfy.display_window import LRRenderer
        HAS_LUNAR_TOOLS = True
        print(f"Found lunar_tools display window at: {lunar_display_path}")
    except ImportError as e:
        print(f"ERROR: Could not import LRRenderer from lunar_tools_comfy.comfy.display_window: {e}")
        sys.exit(1)
else:
    print(f"ERROR: Could not find lunar_tools display window at {lunar_display_path}")
    sys.exit(1)

class GeometryRenderer:
    """Class to handle continuous 3D geometry rendering in a background thread,
    simulating how ComfyUI initializes and runs nodes sequentially but isolated"""
    
    def __init__(self, img_size=512):
        print("Initializing GeometryRenderer...")
        ...
        # Initialize the nodes once, as they would be in ComfyUI
        print("Creating node instances (once, like in ComfyUI)")
        self.node = GeometryRenderNode()
        self.lunar_renderer = LRRenderer()
    ...
    def _render_loop(self):
        ...
        try:
            while self.running:
                ...
                color_tensor = self.node.render(**params)
                ...
                if color_tensor is not None:
                    ...
                    # Update lunar renderer
                    window_title = f"3D Geometry Render - {params['object_type']} [{frame_count}]"
                    print(f"Displaying frame {frame_count} in lunar renderer")
                ...
                # Sleep a bit to avoid using 100% CPU and allow UI to be responsive
                time.sleep(0.03)  # Give UI thread time to process events
